...
def plotBarGraph(x, y, label=''):
    ...
    This method will show the bar graph with the x,y params
    """
...
print(df_elecFromOilCountry['United Kingdom'].aggregate(
    ['mean', 'median', 'max', 'min']))
...
sortedByUkDesc = df_elecFromOilCountry.sort_values(
    by=['United Kingdom'], ascending=False)
...
corr_coeff_coal_oil_2, p_value = pearsonr(df_elecFromOilCountry['Canada'], df_elecFromCoalCountry['Canada'])
corr_coeff_coal_oil_3, p_value = pearsonr(df_elecFromOilCountry['Australia'], df_elecFromCoalCountry['Australia'])
...
plt.plot(df_elecFromOilCountry.index,
         df_elecFromOilCountry["United Kingdom"], label="UK (From Oil)")

plt.plot(df_elecFromCoalCountry.index,
         df_elecFromCoalCountry["United Kingdom"], label="UK (From Coal)")

plt.plot(df_elecFromOilCountry.index,
         df_elecFromOilCountry["Canada"], label="CAN (From Oil)")
...
plt.plot(df_elecFromCoalCountry.index,
         df_elecFromCoalCountry["Australia"], label="AUS (From Coal)")
...
plt.figure(figsize=(8, 6), dpi=1000)
plt.plot(df_popGrowthCountry.index,
         df_popGrowthCountry["United Kingdom"], label="UK Population Growth")
plt.plot(df_popGrowthCountry.index,
         df_popGrowthCountry["Australia"], label="Aus Population Growth")
plt.plot(df_elecFromOilCountry.index,
         df_elecFromOilCountry["Australia"], label="Aus Electricity (From Oil)")

plt.plot(df_elecFromOilCountry.index,
         df_elecFromOilCountry["United Kingdom"], label="UK Electricity (From Oil)")
plt.xlabel("Year")
plt.ylabel("Y axis")
plt.legend()
plt.show()
...
# get correlation of only matching rows but specifying length
corr_coeff_pearson, p_value = pearsonr(
    oil_electricity_uk, pop_growth_uk[:length])

corr_coeff_kendall, p_value = kendalltau(
    oil_electricity_uk, pop_growth_uk[:length])
print(corr_coeff_pearson.round(2))
print(corr_coeff_kendall.round(2))